...
func TestBlockGossipEvent_Decorated(t *testing.T) {
	...
	var (
		...
		metaData       = decorated.Meta.Client.GetEthV1EventsBlockGossip()
		additionalData = decorated.GetEthV1EventsBlockGossip()
	)

	// Assert event.
	require.NotNil(t, decorated)
	require.Equal(t, xatu.Event_BEACON_API_ETH_V1_EVENTS_BLOCK_GOSSIP.String(), event.Type())
...
func TestBlockGossipEvent_Ignore(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
